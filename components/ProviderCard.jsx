...
export default function ProviderCard({ p, onOpen, onToggleFav, isFav, onCompareToggle, comparing }) {
  return (
    <div className="group relative flex h-full flex-col rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition hover:shadow-md">
      ...
      <div className="mt-auto pt-2">
        <button onClick={() => onOpen?.(p)} className="w-full rounded-2xl border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-900 shadow-sm hover:bg-gray-50">Details</button>
      </div>
      <div className="absolute right-3 top-3 flex items-center gap-2 opacity-0 transition group-hover:opacity-100">
        <button onClick={onToggleFav} className={`rounded-full border border-gray-300 bg-white px-2.5 py-1 text-xs font-semibold shadow-sm ${isFav ? "text-amber-700" : "text-gray-700 hover:bg-gray-50"}`}>{isFav ? "★ Saved" : "☆ Save"}</button>
        <button onClick={onCompareToggle} className={`rounded-full border border-gray-300 bg-white px-2.5 py-1 text-xs font-semibold text-gray-700 shadow-sm hover:bg-gray-50 ${comparing ? "ring-2 ring-black" : ""}`}>⇄ Compare</button>
      </div>
    </div>
  );
}
